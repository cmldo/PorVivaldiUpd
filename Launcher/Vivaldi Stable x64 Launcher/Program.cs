...
namespace vivaldi_Stable_x64_Launcher
{
    static class Program
    {
        ...
        static void Main()
        {
            ...
            if (File.Exists(applicationPath + "\\Vivaldi Stable x64\\Vivaldi.exe"))
            {
                ...
                if (!File.Exists(applicationPath + "\\Vivaldi Stable x64\\Profile.txt"))
                {
                    Application.EnableVisualStyles();
                    Application.SetCompatibleTextRenderingDefault(false);
                    Application.Run(new Form1());
                    String Arguments = File.ReadAllText(applicationPath + "\\Vivaldi Stable x64\\Profile.txt") + sb.ToString();
                    if (Arguments.Contains("--user-data-dir="))
                    {
                        ...
                        Process.Start(applicationPath + "\\Vivaldi Stable x64\\Vivaldi.exe", Arguments3);
                    }
                    else
                    {
                        Process.Start(applicationPath + "\\Vivaldi Stable x64\\Vivaldi.exe", Arguments);
                    }
                }
                else
                {
